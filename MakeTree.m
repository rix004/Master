function [nodes] = MakeTree(x_val,y_val,lev,edges,r_vect,x1,y1,theta,distance,levels,root_radius,r_rate,d_rate,delta_theta)
    if levels ~= 1
        ...
        leftside = MakeTree(x_val,y_val,lev,edges,r_vect,x2,y2,theta-(delta_theta+0.5 + 0.5*rand()),distance*d_rate,levels-1,root_radius*r_rate+0.00*rand(),r_rate,d_rate,delta_theta);
        ...
        rightside = MakeTree(x,y,z,e,r,x2,y2,theta+(delta_theta+0.5 + 0.5*rand()),distance*d_rate,levels-1,root_radius*r_rate+0.00*rand(),r_rate,d_rate,delta_theta);
