...
if size(Tree.edges,1) > 0
    ...
    if DistToEdge < d1
            ...
            figure(1)
            plot(x_new,y_new,'*','MarkerSize',10,'Color','r');
            hold on
            ...
            % Find point on the line which is the normal on nearest edge
            % with the right distance
            ratio = DistToEdge/(Tree.LengthRate*Tree.edges(edge_nr,1));
            x = x-(x-x_new)/ratio;
            y = y-(y-y_new)/ratio;
            figure(1)
            plot(x,y,'o','MarkerSize',10,'Color','r');
            hold on
            ...
            NewTree.edges(end,:) = [Tree.LengthRate*Tree.edges(edge_nr,1) size(Tree.nodes,1)+1 size(Tree.nodes,1)+2 Tree.edges(edge_nr,4)*Tree.RadiusRate];
    ...
    % Column 3: Edge number
        ...
        if size(check_edges,1)>0
            ...
            if theta < 90
                ...
                l2 = Tree.edges(edge_nr,1)-l1;
                figure(1)
                plot(x,y,'.','MarkerSize',15,'Color','b');
                % Find point on the line which is the normal on nearest edge
                % with the right distance
                ratio = d1*sind(theta)/(Tree.LengthRate*Tree.edges(edge_nr,1));
                x = x-(x-x_new)/ratio;
                y = y-(y-y_new)/ratio;
                
                plot(x,y,'o','MarkerSize',10,'Color','g');
                hold on
                plot(x_new,y_new,'*','MarkerSize',10,'Color','g');
                hold on
                % Update tree
                NewTree.edges = zeros(size(Tree.edges,1)+2,size(Tree.edges,2));
                NewTree.edges(1:edge_nr,:) = Tree.edges(1:edge_nr,:);
                ...
                % Find point on the line which is the normal on nearest edge
                % with the right distance
                ratio = d1/(Tree.LengthRate*Tree.edges(edge_nr,1));
                x = x-(x-x_near)/ratio;
                y = y-(y-y_near)/ratio;
                % Update tree
                NewTree.nodes = [Tree.nodes;x y];
                NewTree.edges = [Tree.edges;Tree.LengthRate*Tree.edges(edge_nr,1) k size(Tree.nodes,1)+1 Tree.edges(edge_nr,4)*Tree.RadiusRate];
            end
        elseif size(check_edges,1) == 0
            ...
            % the combinated tree).
            ratio = d1/Tree.StartLength;
            x = x-(x-x_near)/ratio;
            y = y-(y-y_near)/ratio;
            % Update tree
            NewTree.nodes = [Tree.nodes;x y];
            NewTree.edges = [Tree.edges;Tree.StartLength k size(Tree.nodes,1)+1 Tree.TrunkRadius];
        else
