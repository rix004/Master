function [V,C,XY]=VoronoiLimit_endret(varargin)
% --------------------------------------------------------------
    ...
    for ij=1:length(C)
        if sum(allVixinp(C{ij}))~=length(C{ij})
            ...
            if poly_ok(ij)==1
                ...
                A = polyshape(crs(:,1),crs(:,2),'Simplify',false);
                B = polyshape(V(C{ij},1),V(C{ij},2),'Simplify',false);
                C = intersect(A,B);
                [xb,yb] = boundary(C);
...
%                                     nb=nb(poly_ok(nb)==0);
                                    if ~isempty(nb)
                                        ...
                                        for in=1:length(nb)
                                            ...
                                            A = polyshape(V(C{nb(in)},1),V(C{nb(in)},2),'Simplify',false);
                                            B = polyshape(poly(:,1),poly(:,2),'Simplify',false);
                                            C = intersect(A,B);
                                            [xb,~] = boundary(C);
                                            if ~isempty(xb)
                                                overlap(in)=1;
    ...
    if exist('bs_int','var')
        ...
        for ii=1:length(bs_int)
            ...
            for ij=1:length(C)
                ...
                A = polyshape(V(C{ij},1),V(C{ij},2),'Simplify',false);
                B = polyshape(bs_int{ii}(:,1),bs_int{ii}(:,2),'Simplify',false);
                C = subtract(A,B);
                [pbx,pby]=boundary(C);
                if ~isempty(pbx)
