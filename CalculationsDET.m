...
% How many times to change level difference
iterations = 1;

% How many times to change M
iterations1 = 1;    
...
%     end 
    ...
    save('./Filer/PressurePlotDataDET_KD08','p_darcyEx','p_networkEx','Tree','TNinfo','D','cell_center','boundary_cells','Bv_darcy');
    ...
    save('./Filer/CoarsePressurePlotDataDET_KD08','p_darcyCoarse','p_network','DETtree','TNinfo','D','cell_center','boundary_cells','Bv_darcy');
    ...
    error(iter,iter1) = sqrt(e/D_area)/(max(p_darcyEx)-min(p_darcyEx));
    sqrt(e/D_area)
    (max(p_darcyEx)-min(p_darcyEx))
    % Change when testing (x-axis in RMS-plot)
    delta_m(iter,iter1)=DT.RadiusRate;
...
%%%% PLOTS %%%%
%save('VaryLevelDiffAndKD_rel2pressure','iterations1','delta_m','error');
%save('VaryMAndLevelDiff_rel2pressure','iterations1','delta_m','error');
%save('VaryMAndLevelDiff_rel2pressure','iterations1','delta_m','error');
%Plots('ErrorPlot_M_deltaM','VaryMAndLevelDiff_rel2pressure')
