function[p_darcy,q_T,q_network,p_network]=SolveSystemEx(nodes,edges,TNinfo,TNlogic,Dir_network,Neu_network,mu,k,K_N,LHS,RHS,cell_area,BC)
Ne = length(edges);
Nn = length(nodes);
...
if strcmp(BC, 'Dirichlet')
    ...
    P = zeros(Ntn,Ne);
    for i = 1:size(P,1)
        P(i,TNinfo(i,3))=mu/(2*k*pi)*log(sqrt(cell_area(i))*0.2/edges(TNinfo(i,3),4));
    end
    P = sparse(P);

    PM_vect = zeros(Ntn,1);
    for i = 1:size(PM_vect,1)
        PM_vect(i)=mu/(2*k*pi)*log(sqrt(cell_area(i))*0.2/edges(TNinfo(i,3),4));
    end
    ...
    I, sparse(Ntn,Ne), sparse(Ntn,Nin), -I, sparse(Ntn,Ntn)];
    ...
    p_network(Bc_nodes+TNlogic==0) = p_int;
...
elseif strcmp(BC, 'Neumann')
    ...
    P = zeros(Ntn,Ne-1);
    for i = 1:size(P,1)
        P(i,TNinfo(i,3)-1)=mu/(2*k*pi)*log(sqrt(cell_area(i))*0.2/edges(TNinfo(i,3),4));
    end
    P = sparse(P);
