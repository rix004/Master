...
map = flip(turbo(7),1);
    ...
    xlabel('R [mm]','FontSize',axisfontsize)
    lgd = legend('8000 particles','','','4000 particles','','','2000 particles','','','1000 particles')
...
if strcmp(ThisFigure,'ErrorPlotDifferentValuesDLA')
    ...
    for i = 1:size(x,1)
        for j = 1:size(x,2)
            data_mean(i,j)=mean([e1(i,j);e2(i,j);e3(i,j)]);
            data_std(i,j)=std([e1(i,j);e2(i,j);e3(i,j)]);
        end
    end
...
if strcmp(ThisFigure,'ErrorPlotDifferentValuesRRT')
    ...
    for i = 1:size(x,1)
        for j = 1:size(x,2)
            data_mean(i,j)=mean([e1(i,j);e2(i,j);e3(i,j);e4(i,j);e5(i,j);e6(i,j)]);
            data_std(i,j)=std([e1(i,j);e2(i,j);e3(i,j);e4(i,j);e5(i,j);e6(i,j)]);
        end
    end
    rgb=zeros(size(x,2),3);
    rgb(1,:)=[0 0.4470 0.7410];
